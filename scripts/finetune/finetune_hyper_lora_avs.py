...
def train(attn_implementation=None):
    ...
    seg_ckpt_dir = training_args.pretrain_ckpt_dir
    
    ckpt = torch.load(join(training_args.finetune_ckpt_dir,'finetune_weights.bin'),map_location='cpu')
    model.load_state_dict(ckpt,strict=False)
    print(f'load hyper_lora weights finished...')

    ckpt = torch.load(join(seg_ckpt_dir,'segmentation_pretrain.bin'),map_location='cpu')
    model.model.load_state_dict(ckpt,strict=False)
    print('load pretrained seg_module ckpt finished...')
